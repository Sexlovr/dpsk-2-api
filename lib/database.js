...
export async function initDB() {
    ...
    // ── Migrate conversations table to state-hash schema ──
    // Old schema had `conv_key` + UNIQUE(conv_key, api_key_hash) which collides
    // when two parallel chats share the same character/preset. Replace it.
    const convCols = db.exec("PRAGMA table_info(conversations)");
    const hasOldShape = convCols.length && convCols[0].values.some(r => r[1] === 'conv_key');
    const hasStateHash = convCols.length && convCols[0].values.some(r => r[1] === 'state_hash');
    if (!convCols.length || hasOldShape || !hasStateHash) {
    ...
    db.run(`
        ...
        CREATE INDEX IF NOT EXISTS idx_conv_lookup
            ON conversations(api_key_hash, state_hash, last_used);
    `);
